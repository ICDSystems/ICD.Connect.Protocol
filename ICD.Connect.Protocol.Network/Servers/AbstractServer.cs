...
namespace ICD.Connect.Protocol.Network.Servers
{
	public abstract class AbstractServer<TClient> : IServer<TClient>
	{
		...
		private readonly IcdSortedDictionary<uint, TClient> m_Clients;
		...
		protected AbstractServer()
		{
			...
			m_Clients = new IcdSortedDictionary<uint, TClient>();
		...
		/// <returns></returns>
		public bool TryGetClientInfo(uint client, out TClient info)
		{
			m_ClientsSection.Enter();
		...
		protected void AddClient(uint clientId, TClient hostInfo, SocketStateEventArgs.eSocketStatus reason)
		{
			m_ClientsSection.Enter();
		...
		{
			addRow("Name", Name);
			addRow("Number of Clients", NumberOfClients);
		...
		{
			TableBuilder builder = new TableBuilder("Client ID", "Info");
