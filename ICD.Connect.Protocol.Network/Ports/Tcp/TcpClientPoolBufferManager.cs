...
{
	/// <summary>
	/// Creates and manages buffers for each TCP client that is generated by a TcpClientPool.
	/// </summary>
	public sealed class TcpClientPoolBufferManager : IDisposable
	{
		public delegate void ClientCompletedSerial(TcpClientPoolBufferManager sender, IcdTcpClient client, string data);
		...
		private readonly BiDictionary<IcdTcpClient, ISerialBuffer> m_Buffers;
		...
		public TcpClientPoolBufferManager(Func<ISerialBuffer> bufferFactory)
		{
			...
			m_Buffers = new BiDictionary<IcdTcpClient, ISerialBuffer>();
		...
		public void Clear()
		{
			IcdTcpClient[] keys = m_BufferSection.Execute(() => m_Buffers.Keys.ToArray());
			foreach (IcdTcpClient key in keys)
				RemoveBuffer(key);
		}
		...
		/// <returns></returns>
		private ISerialBuffer LazyLoadBuffer(IcdTcpClient client)
		{
		...
		/// </summary>
		/// <param name="client"></param>
		private void RemoveBuffer(IcdTcpClient client)
		{
			m_BufferSection.Enter();
		...
		private void ClientOnSerialDataReceived(TcpClientPool tcpClientPool, IcdTcpClient client, string data)
		{
		...
		private void ClientOnConnectedStateChanged(TcpClientPool tcpClientPool, IcdTcpClient client, bool connected)
		{
		...
		{
			IcdTcpClient client = m_BufferSection.Execute(() => m_Buffers.GetKey(sender as ISerialBuffer));
