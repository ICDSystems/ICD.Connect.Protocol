...
namespace ICD.Connect.Protocol.Network.Ports.TcpSecure
{
	...
	{
		private SecureTCPClient m_TcpClient;
		...
		public override void Connect()
		{
			...
			try
			{
				m_TcpClient = new SecureTCPClient(Address, Port, BufferSize);
...
#region TCPClient Callbacks
		...
		private void Subscribe(SecureTCPClient tcpClient)
		{
		...
		/// </summary>
		/// <param name="tcpClient"></param>
		private void Unsubscribe(SecureTCPClient tcpClient)
		{
			tcpClient.SocketStatusChange -= TcpClientOnSocketStateChange;
		}
		...
		private void TcpClientReceiveHandler(SecureTCPClient tcpClient, int bytesReceived)
		{
			try
			{
				if (bytesReceived <= 0)
