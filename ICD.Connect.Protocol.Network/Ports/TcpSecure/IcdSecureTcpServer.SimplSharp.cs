...
namespace ICD.Connect.Protocol.Network.Ports.TcpSecure
{
	public sealed partial class IcdSecureTcpServer
	{
		...
		private static X509Certificate s_ServerCertificate;
		...
		/// A null or empty path indicates an unspecified cert and a new one will be auto-generated.
		/// </summary>
		/// <param name="path">The path to read a certificate file from</param>
		public void SetCertificate(string path, string uuid)
		{
			try
			{
				// Unspecified cert, auto-generate a cert to use.
				if (string.IsNullOrEmpty(path))
				{
					var autoCert = X509Utils.GenerateCertificate(ICD_SECURE_TCP_SERVER_COMMON_NAME);
					string autoPath = WriteBouncyCastleCertFile(autoCert, uuid);
					var autoCertFile = ReadCertFileToByteArray(autoPath);
					var autoCertificate = new X509Certificate(autoCertFile);
					if (!IsValidCertificate(autoCertificate))
						throw new InvalidOperationException("Auto-generated certificate is invalid");

					s_ServerCertificate = autoCertificate;
				}
				// Load the specified cert.
				else
				{
					byte[] certFile = ReadCertFileToByteArray(path);
					var certificate = new X509Certificate(certFile);
					if (!IsValidCertificate(certificate))
						throw new InvalidOperationException("Specified certificate is invalid");

					s_ServerCertificate = certificate;
				}

				m_TcpListener.SetServerCertificate(s_ServerCertificate);
			}
			catch (Exception e)
			{
		...
		private static byte[] ReadCertFileToByteArray(string path)
		{
			byte[] buffer;
