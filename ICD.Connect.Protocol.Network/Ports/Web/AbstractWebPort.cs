...
namespace ICD.Connect.Protocol.Network.Ports.Web
{
		...
		where TSettings : IWebPortSettings, new()
	{
		...
		public abstract bool Get(string relativeOrAbsoluteUri, IDictionary<string, List<string>> headers, IDictionary<string, List<string>> body, out string response);
		...
		/// <param name="response"></param>
		/// <returns></returns>
		public abstract bool Post(string relativeOrAbsoluteUri, string data, Encoding encoding, out string response);
