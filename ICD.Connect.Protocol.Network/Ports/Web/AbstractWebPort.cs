...
namespace ICD.Connect.Protocol.Network.Ports.Web
{
		...
		where TSettings : IWebPortSettings, new()
	{
		...
		/// <param name="data"></param>
		/// <returns></returns>
		public WebPortResponse Put(string relativeOrAbsoluteUri, byte[] data)
		{
			return Put(relativeOrAbsoluteUri, new Dictionary<string, List<string>>(), data);
		}
		...
		/// <param name="data"></param>
		/// <returns></returns>
		public abstract WebPortResponse Put(string relativeOrAbsoluteUri, Dictionary<string, List<string>> headers, byte[] data);
